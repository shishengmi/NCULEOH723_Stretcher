...
SemaphoreHandle_t 	WifiSemHandle;

extern uint8_t wifiUserBuffer[1024];
...
//#########################################################################################################
bool	Wifi_SendRaw_DMA(uint8_t *data,uint16_t len)
{
  if(len <= _WIFI_TX_SIZE)
  {
    memcpy(Wifi.TxBuffer,data,len);
    if(HAL_UART_Transmit_DMA(&_WIFI_USART,data,len) == HAL_OK)
      return true;
    else
      return false;
  }
...
////    // 等待信号量，表示有数据需要发送
  if (i>=2024)
  {
    Wifi_TcpIp_SendDataUdp(1, 1024, wifiUserBuffer);
    osDelay(100);
  }
}
...
void	Wifi_Init(osPriority	Priority)
{
	...
	Wifi_RxClear();
	Wifi_TxClear();/*清除缓冲区*/
  WifiSemHandle = xSemaphoreCreateMutex();
  xTaskCreate(WifiTask,"Wifi Task",2048,NULL,Priority,&WifiTaskHandle);
}
//#########################################################################################################
//#########################################################################################################
//#########################################################################################################
